...
read n
if [ $n -eq 1 ]; then
    echo -e "正在进入|1\t|本地\t|ROOT\t\t|管理员"
    mysql -u root -p
elif [ $n -eq 2 ]; then
    echo -e "正在进入|2\t|本地\t|USER\t\t|用户"
    mysql -u user -p
elif [ $n -eq 3 ]; then
    echo -e "正在进入|3\t|本地\t|WORKPRESSUSER\t|workpress管理员"
    mysql -u workpressuser -p
elif [ $n -eq 4 ]; then
    echo -e "正在进入|4\t|本地\t|HIVEUSER\t|hive管理员"
    mysql -u hiveuser -p
elif [ $n -eq 5 ]; then
    echo -e "正在进入|5\t|RDS\t|RDS_ROOT\t|管理员"
    mysql -h rm-2ze2pr85o2ozj8479po.mysql.rds.aliyuncs.com -P 3306 -u rds_root -p
elif [ $n -eq 6 ]; then
    echo -e "正在进入|6\t|RDS\t|RDS_USER\t|用户"
